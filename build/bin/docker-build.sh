...
if [[ "$TARGET_PLATFORM" == "amd64" ]]; then
  if [[ "$OSCAP_ENABLED" != "true" ]]; then
    ...
    # To do: Getting permissions denied while uploading the $image_name-remediation.sh to artifactory since the file has readonly permissions only to root user
