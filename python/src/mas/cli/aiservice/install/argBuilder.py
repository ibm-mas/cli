...
class aiServiceInstallArgBuilderMixin():
    def buildCommand(self) -> str:
        ...
        if self.getParam('aiservice_db_host') != "":
            command += f"  --aiservice-db-host \"{self.getParam('aiservice_db_host')}\"{newline}"
        if self.getParam('aiservice_db_port') != "":
            command += f"  --aiservice-db-port \"{self.getParam('aiservice_db_port')}\"{newline}"
        if self.getParam('aiservice_db_user') != "":
            command += f"  --aiservice-db-user \"{self.getParam('aiservice_db_user')}\"{newline}"
        if self.getParam('aiservice_db_database') != "":
            command += f"  --aiservice-db-database \"{self.getParam('aiservice_db_database')}\"{newline}"
        if self.getParam('aiservice_db_secret_name') != "":
            command += f"  --aiservice-db-secret-name \"{self.getParam('aiservice_db_secret_name')}\"{newline}"
        if self.getParam('aiservice_db_secret_key') != "":
            command += f"  --aiservice-db-secret-key \"{self.getParam('aiservice_db_secret_key')}\"{newline}"
        if self.getParam('aiservice_db_secret_value') != "":
            command += f"  --aiservice-db-secret-value \"{self.getParam('aiservice_db_secret_value')}\"{newline}"
