...
- **Migration from IBM User Data Services Operator (UDS) to IBM Data Reporter Operator (DRO)** UDS is being deprecated in June, Starting With [February 2024](v8-240227-amd64.md) Maximo Operator catalog MAS will be moving to use DRO as a replacement to UDS.  Running `mas update` will migrate and configure all existing MAS 8.10x and MAS 8.11.x instances from UDS to DRO. 
