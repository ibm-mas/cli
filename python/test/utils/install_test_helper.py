...
class InstallTestHelper:
    ...
    def __init__(self, tmpdir, config: InstallTestConfig):
        """
...
class AiServiceInstallTestConfig:
    """Configuration for an aiservice install test scenario."""
    ...
    def __init__(
        ...
        """
        Initialize aiservice test configuration.
...
class AiServiceInstallTestHelper:
    """Helper class to run aiservice install tests with minimal code duplication."""

    def __init__(self, tmpdir, config: AiServiceInstallTestConfig):
        """
        Initialize the aiservice test helper.
    ...
    def run_install_test(self):
        """
        Run the aiservice install test with all mocks configured.
        ...
        with mock.patch('mas.cli.cli.config'):
            ...
            with (
                ...
                mock.patch('mas.cli.aiservice.install.app.launchInstallPipeline') as launch_install_pipeline,
                mock.patch('mas.cli.cli.isSNO') as is_sno,
                mock.patch('mas.cli.displayMixins.prompt') as mixins_prompt,
                mock.patch('mas.cli.displayMixins.PromptSession') as prompt_session_class,
                ...
                mock.patch('mas.cli.aiservice.install.app.getDefaultStorageClasses') as get_default_storage_classes
            ):
                # Configure mock return values
                dynamic_client_class.return_value = dynamic_client
                get_nodes.return_value = [{'status': {'nodeInfo': {'architecture': self.config.architecture}}}]
...
def run_aiservice_install_test(tmpdir, config: AiServiceInstallTestConfig):
    """
    ...
    Raises:
        ...
        AssertionError: If prompt verification fails
    """
    helper = AiServiceInstallTestHelper(tmpdir, config)
    helper.run_install_test()
