...
Maximo Visual Inspection v9.1.5
================================================================================
...
- Add ObjectStore client connection parameters to custom-service ConfigMap (#926)
- Add flag to toggle ReadOnlyRootFilesystem (#922)
- Fix CRD version conflicts for spec.settings.readOnlyRootFilesystem - backport 9.1.x (#937)
- [patch] Fix cannot create temporary files when upload files on readOnlyRootFilesystem - backport 9.1.x (#940)
- Fix import failures for datasets and models on IBM Cloud read-only clusters (#959)
- increase vision-service images cache size (9.1.x) (#961)
- Address kyverno policy topologyspreadconstraints (#962)
- Address RunAsNonRoot kyverno for task-pvc (ODF only) (#954)
