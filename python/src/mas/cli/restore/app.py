...
class RestoreApp(BaseApp):
    ...
    # When include_dro is true, set the dro_cfg_file to the default mounted path and include_drocfg_from_backup to false
    # this is because when DRO is installed, suite-restore will use the cfg generated by the dro install task.
    def setNonInteractiveDROParams(self):
    ...
    def promptForBackupStorage(self) -> None:
        ...
        # Check if this is SNO
        isSNO = self.isSNO()

        if isSNO:
            self.printDescription([
                " - <Yellow>Single Node OpenShift (SNO) detected</Yellow>",
    ...
    def createConfigSecrets(self) -> None:
        ...
        include_dro = self.getParam("include_dro")
        ...
        if slsCfgFile or droCfgFile:
            ...
            if include_dro != "true" and droCfgFile and not path.exists(droCfgFile):
